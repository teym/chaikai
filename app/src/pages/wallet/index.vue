    ...
    <div class="content col flex">
      <navbar :fixed="true">
        <div class="navbar row i-center">
          <div class="flex i-center pad2-l" @click="onBack">
            <img src="/static/images/arrow_left_white.png" alt="back" />
          </div>
      ...
      <div class="flex white_bg round pad2-l pad2-r">
        ...
        <div class="list">
          <p class="small light margin-t">
            报名参加活动，合作模式选择【接受悬赏】【达人报价】，达人完成合作任务后，可获得一定的悬赏
          </p>
        </div>
        ...
        <div class="list">
          ...
          <p class="small light margin-t">
            2.所有已发放的悬赏均可提现，每月可提现一次；
          </p>
          <p class="small light margin-t">
            3.当日提交的提现申请，将在24小时内，提现至微信钱包；
          </p>
          <p class="small light margin-t">
            4.因合作过程有歧义导致发放悬赏金额有误，可联系客服解决；
          </p>
          <p class="small light margin-t">
  ...
  methods: {
    ...
    onGo () {
      uiapi
        .alert({
          title: '提现确认',
          content: `确认提现当前悬赏月：${this.amount}元，确认将消耗一次提现次数`
        })
        .then((r) => {
          const l = uiapi.loading()
          request
            .post('/bl/account/finance/withdraw', { amount: this.amount })
            .then((r) => {
              uiapi.toast('提现成功')
              this.loadData()
              setTimeout(() => {
