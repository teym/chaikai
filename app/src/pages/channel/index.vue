      ...
      <ul class="white_bg channel">
        <li v-for="(item, i) in bind" :key="i" class="row i-center pad2">
          <div class="row i-center" @click="onItem(item)">
            ...
            <span class="light small margin-l"
              >粉丝数：{{ item.fansCount || 0 }}</span
            >
            <img
              class="right"
  ...
  methods: {
    loadData () {
      return request
        ...
        .then(({ json: { data } }) => {
          this.data = data
          const channels = _.partition(
            mapChannel(data.channels),
            (i) => i.statusCode === 3
          )
          ...
          if (!(this.data.telephone && this.data.realName && this.data.wechatNo)) {
            uiapi
              .alert({
                title: '温馨提示',
                content: '为核实认证渠道的信息真实性，请先完善个人信息',
                confirmText: '去完善',
                confirmColor: '#ff8e3b'
              })
              .then((r) => {
    ...
    onUnbind (item) {
      const l = uiapi.loading()
      request
        .post('/bl/account/channel/unbind', item)
        .then((r) => {
          l()
          uiapi.toast('解绑成功')
          this.loadData()
