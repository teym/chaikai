<template>
  <div class="container white_bg pos_r">
    ...
    <div class="col head i-center pad2">
      ...
      <p class="small light margin2">
        拆开是一个集美妆、个护、美食、探店的测评合作平台，活动报名成功即可免费带产品回家，完成任务合作，还可获得悬赏哦～
      </p>
    </div>
    <div class="flex col i-center">
      <button
        v-if="useprofile"
        :disabled="!checked"
        class="btn row center middle blod"
        @click="onLogin"
      >
        微信授权登录
      </button>
      ...
      <span class="middle light margin2-t cancel" @click="onCancel"
        >取消登录</span
      >
    </div>
    <div class="row center pad2 bar">
      ...
      <p class="small light">
        <span class="red" @click="onPro(1)">《服务协议》</span>及<span
          class="red"
          @click="onPro(2)"
          >《隐私政策》</span
        >
...
export default {
  ...
  created () {
    this.useprofile = !!wx.getUserProfile
    this.refreshCode()
  },
  beforeDestroy () {
  ...
  methods: {
      ...
      if (this.useprofile) {
        api
          ...
          .then((r) => {
            ...
            return api.rawLogin(this.code, r)
          })
          .then(() => {
    ...
    onPro (t) {
      router(this).push('/pages/web/main', {
        url: request.rawConf().web + (t === 1 ? '?id=QWxLV' : '?id=W5pLA')
      })
    },
    onCancel (e) {
      router(this).pop()
