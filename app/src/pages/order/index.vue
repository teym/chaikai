<template>
  <div class="container light_bg">
    <div class="white_bg pad2 row">
      ...
      <img class="state margin2" v-if="data.statusCode === 8" src="/static/images/issue_status_3.png" alt="status" />
    </div>
...
export default {
  ...
  methods: {
    ...
    onCancel () {
      uiapi.alert({ title: '取消申请', content: '取消申请活动' })
        .then(r => {
          const l = uiapi.loading()
    ...
    onPay () {
      const l = uiapi.loading()
      request.post('/wxpay/mini', {amount: this.data.depositInfo.amount || this.data.goods.price, brActivityId: this.data.activity.id, brActivityOrderId: this.data.id, payScene: this.data.statusCode >= 2 ? 'BL_PAY_DEPOSIT_AFTER' : 'BL_PAY_DEPOSIT'}).then(({json: {data}}) => {
        return api.pay(data)
      }).then(r => {
        setTimeout(() => {
          l()
