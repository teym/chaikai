...
export default {
  ...
  methods: {
    ...
    onPay () {
      const l = uiapi.loading()
      request.post('/wxpay/mini', {amount: 0.01 /* this.data.depositInfo.amount || this.data.goods.price */, brActivityId: this.data.activity.id, brActivityOrderId: this.data.id, payScene: this.data.statusCode >= 2 ? 'BL_PAY_DEPOSIT_AFTER' : 'BL_PAY_DEPOSIT'}).then(({json: {data}}) => {
        return api.pay(data)
      }).then(r => {
        setTimeout(() => {
          l()
